...
func Render(p facts.Provider, jsonldURL, agentCardURL, mcpManifestURL, mcpURL string) string {
	return fmt.Sprintf(
		"# %s\n\n> Live, agent-queryable availability and eligibility facts published by %s.\n\n"+
			...
			"- [MCP manifest](%s)\n"+
			"- [MCP endpoint](%s)\n",
		p.Name, p.Name, renderProperties(p.Properties), jsonldURL, agentCardURL, mcpManifestURL, mcpURL,
	)
}
